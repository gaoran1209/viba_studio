...
import { Shirt, Loader2, ArrowDown, Sparkles, X, Maximize2, Download } from 'lucide-react';
...
export const TryOnView: React.FC = () => {
  ...
  const handleGenerate = async () => {
    if (!modelInput || !clothInput) return;
    ...
    const newJob: TryOnJob = {
      ...
      modelFile: modelInput,
      clothFile: clothInput,
      modelPreview: URL.createObjectURL(modelInput),
      clothPreview: URL.createObjectURL(clothInput),
      status: 'processing',
      ...
      <div className="bg-white border-b border-gray-200 p-6 shrink-0 z-10 shadow-sm">
        <div className="max-w-6xl mx-auto w-full flex flex-col gap-6">
          <div className="flex justify-between items-end">
            <div>
              <h2 className="text-xl font-bold text-gray-900">{t.try_on.title}</h2>
              <p className="text-sm text-gray-500">{t.try_on.desc}</p>
            </div>
            
             <button 
                onClick={handleGenerate}
                disabled={!modelInput || !clothInput || isSubmitting}
                className={`px-8 py-2.5 rounded-full font-medium flex items-center gap-2 transition-all shadow-md active:scale-95 ${
                  !modelInput || !clothInput || isSubmitting
                    ? 'bg-gray-100 text-gray-400 cursor-not-allowed shadow-none'
                    : 'bg-blue-600 text-white hover:bg-blue-700'
                }`}
              >
                {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : <ArrowDown size={18} />}
                {isSubmitting ? t.common.processing : t.try_on.btn}
              </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="h-[280px] bg-white rounded-2xl border border-gray-200 p-4 flex flex-col shadow-sm relative group hover:border-blue-200 transition-colors">
              <div className="flex items-center gap-2 mb-3 pb-3 border-b border-gray-50">
                 ...
                 <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">{t.try_on.step1}</span>
              </div>
              <div className="flex-1 min-h-0">
                <ImageUpload 
                  label={t.try_on.step1_label} 
                  onImageSelect={setModelInput} 
                  selectedImage={modelInput}
                  onClear={() => setModelInput(null)}
                  heightClass="h-full"
                />
              </div>
            </div>
            ...
            <div className="h-[280px] bg-white rounded-2xl border border-gray-200 p-4 flex flex-col shadow-sm relative group hover:border-purple-200 transition-colors">
              <div className="flex items-center gap-2 mb-3 pb-3 border-b border-gray-50">
                 ...
                 <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">{t.try_on.step2}</span>
              </div>
              <div className="flex-1 min-h-0">
                <ImageUpload 
                  label={t.try_on.step2_label} 
                  subLabel={t.try_on.step2_sub}
                  onImageSelect={setClothInput} 
                  selectedImage={clothInput}
                  onClear={() => setClothInput(null)}
                  heightClass="h-full"
                />
              </div>
            </div>
